...
char* TrampHook(char* src, char* dst, unsigned int len) {
	if (len < 5) return 0;
	char* gateway = (char*)VirtualAlloc(0, len + 5, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE); // making the gateway
	memcpy(gateway, src, len); 
	uintptr_t jumpAddy = (uintptr_t)(src - gateway - 5); // jump
	*(gateway + len) = (char)0xE9;
	*(uintptr_t*)(gateway + len + 1) = jumpAddy;
	if (Hook(src, dst, len)) {
		return gateway; // returns back to the gateway to contuine with origonal code 
	}
	else return nullptr;
}